...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            if (self.badgeImage != nil && self.badgeIndex == idx){
                CALayer *imageLayer = [CALayer layer];
                imageLayer.frame = CGRectMake(rect.origin.x + rect.size.width, rect.origin.y-5, self.badgeImage.size.width, self.badgeImage.size.height);
...
- (void)setHotImage:(UIImage *)image withIndex:(NSInteger)index {
